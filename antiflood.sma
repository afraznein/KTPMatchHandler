...
public chkFlood(id)
{
	...
	if (maxChat)
	{
		// KTP Modification: Use system time instead of game time for flood checking
		// Game time freezes during pause, causing all messages after first to be blocked
		// System time continues during pause, allowing proper flood checking
		new Float:nexTime = float(get_systime())
